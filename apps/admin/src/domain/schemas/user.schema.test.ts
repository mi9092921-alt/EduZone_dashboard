...
describe('user domains schemas', () => {
  ...
  it('validates suspendUserSchema', () => {
    expect(suspendUserSchema.safeParse({ reason: 'ok reason', suspend_hours: 0 }).success).toBe(
      false,
    );
    expect(suspendUserSchema.safeParse({ reason: 'ok reason', suspend_hours: 48 }).success).toBe(
      true,
    );
  });

  it('validates banUserSchema', () => {
    expect(banUserSchema.safeParse({ reason: 'ok reason', confirm_text: 'ban' }).success).toBe(
      false,
    );
    expect(banUserSchema.safeParse({ reason: 'ok reason', confirm_text: 'BAN' }).success).toBe(
      true,
  ...
  it('validates issueWarningSchema', () => {
    ...
    expect(
      issueWarningSchema.safeParse({
        reason: 'This is a valid long reason for a warning',
        severity: 2,
      }).success,
    ).toBe(true);
  });
