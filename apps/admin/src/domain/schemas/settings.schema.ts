...
export const getSetSettingSchema = (t: Translator) => z.object({
  key: z.string().min(1, t('validation.key_required')),
  value: z.string(),
  value_type: z.enum(['string', 'integer', 'boolean', 'json']).default('string'),
});

export const getMaintenanceModeSchema = (t: Translator) => z.object({
...
export const getAppLockSchema = (t: Translator) => z.object({
  message: z.string().min(5, t('validation.message_min', { min: 5 })).max(2000),
});

// ── Feature Flags ────────────────────────────────────────────

export const getCreateFeatureFlagSchema = (t: Translator) => z.object({
  key: z
    .string()
...
export const getUpdateFeatureFlagSchema = (_t: Translator) => z.object({
  label: z.string().max(200).optional(),
  description: z.string().max(1000).optional(),
