...
// ── Status config ────────────────────────────────────────────────
const STATUS_CONFIG = {
  published: { color: 'text-emerald-700 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-100 dark:border-emerald-500/20', dot: 'bg-emerald-500 dark:bg-emerald-400' },
  draft: { color: 'text-slate dark:text-slate-300', bg: 'bg-slate-100 dark:bg-slate-500/10 border-slate-300 dark:border-slate-500/20', dot: 'bg-slate-600 dark:bg-slate-400' },
  archived: { color: 'text-rose-700 dark:text-rose-400', bg: 'bg-rose-50 dark:bg-rose-500/10 border-rose-100 dark:border-rose-500/20', dot: 'bg-rose-500 dark:bg-rose-400' },
} as const;
...
export function CoursesTable({
  ...
  const someSelected = courses.length > 0 && courses.some(c => selectedIds.includes(c.id)) && !allSelected;
        ...
        <table className="w-full text-start border-separate border-spacing-0 min-w-[900px]">
          ...
          <tbody className="divide-y divide-border">
            {isLoading
              ...
              : courses.map((course) => {
                const status = STATUS_CONFIG[course.status as keyof typeof STATUS_CONFIG] ?? STATUS_CONFIG.draft;
                ...
                return (
                  ...
                  >
                    {/* Checkbox */}
                    <td className="px-4 py-5 border-b border-border/40 text-center" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-center">
                        <input
                          type="checkbox"
