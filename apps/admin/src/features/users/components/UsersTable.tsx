...
const UserRow = memo(
  ({
    ...
    const statusConfig =
      STATUS_CONFIG[user.account_status as keyof typeof STATUS_CONFIG] || STATUS_CONFIG.active;
    const roleClass =
      ROLE_CONFIG[user.primary_role as keyof typeof ROLE_CONFIG] || ROLE_CONFIG.student;
    ...
    return (
      ...
      >
        <td
          className="px-5 py-4 sticky left-0 bg-card group-hover:bg-muted/30 z-10 border-b border-border/40"
        ...
        <td className="px-4 py-4 border-b border-border/40">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-bold text-foreground truncate max-w-[260px] capitalize">
              {displayName}
            </span>
          </div>
        </td>
        <td className="px-4 py-4 text-start border-b border-border/40">
          <div className="flex flex-col gap-0.5 w-fit">
            <div className="flex items-center gap-1 group/contact w-fit">
              <span className="text-sm text-foreground/80 font-medium truncate max-w-[280px]">
                {user.email || '—'}
              </span>
        ...
        <td className="px-4 py-4 border-b border-border/40">
          <span
            className={cn(
              'px-2 py-0.5 rounded-md border text-[10px] font-extrabold uppercase tracking-tight',
              roleClass,
            )}
          >
        ...
        <td className="px-4 py-4 border-b border-border/40">
          <div
            className={cn(
              'inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-tight border transition-faang',
              statusConfig.bg,
              statusConfig.color,
            )}
          >
        ...
        <td className="px-4 py-4 text-sm text-muted-foreground whitespace-nowrap border-b border-border/40">
          {user.last_login
            ? formatDistanceToNow(user.last_login, locale)
            : t('temp_password_placeholder')}
        </td>
        <td
          className="px-5 py-4 text-end sticky right-0 bg-card group-hover:bg-muted/30 z-10 border-b border-border/40 shadow-[-8px_0_12px_-6px_rgba(0,0,0,0.05)] overflow-visible"
          onClick={(e) => e.stopPropagation()}
        >
          <UserRowActions
            user={user}
          ...
          <tbody className="divide-y divide-border/40">
            {isLoading ? (
              skeletonRows.map((i) => (
                <tr key={i} className="animate-pulse">
                  ...
                  <td className="px-5 py-4 text-end">
                    <div className="h-8 w-8 bg-muted rounded-full ms-auto" />
                  </td>
                </tr>
              ))
            ) : users.length === 0 ? (
              <tr>
                <td
                  colSpan={7}
                  className="px-4 py-12 text-center text-muted-foreground text-sm font-medium"
                >
                  {t('no_users_found')}
                </td>
              </tr>
