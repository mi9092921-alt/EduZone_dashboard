...
/**
 * Audit service — browser-safe Supabase queries for the audit chain domain.
 * No UI, no React — pure async functions.
 *
 * P1 FIX (server/client boundary): this module MUST NOT import the
 * service-role client — it is bundled for client components/mutations/
 * queries. Privileged reads (`getActivityLogsAdmin`, `getQueuedActivities`)
 * live in `./audit.admin` (server-only).
 */
