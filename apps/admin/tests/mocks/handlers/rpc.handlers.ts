...
export const rpcHandlers = [
  ...
  http.post('*/rest/v1/rpc/get_users', () =>
    HttpResponse.json([
      { id: MOCK_USER_ID, email: 'user@test.com', primary_role: 'student', account_status: 'active', tenant_id: MOCK_TENANT_ID, warning_count: 0, shard_key: 1 },
      { id: '00000000-0000-0000-0000-000000000003', email: 'user2@test.com', primary_role: 'teacher', account_status: 'active', tenant_id: MOCK_TENANT_ID, warning_count: 0, shard_key: 1 },
    ]),
  ),
  ...
  http.post('*/rest/v1/rpc/control_user_account', async ({ request }) => {
    ...
    return HttpResponse.json({
      ...
      suspension_until:   action === 'suspend' ? new Date(Date.now() + 48 * 3600_000).toISOString() : null,
    });
  }),
  ...
  // ── reset_user_device ────────────────────────────────────────────
  http.post('*/rest/v1/rpc/reset_user_device', () =>
    HttpResponse.json({ deactivated_count: 2 }),
  ),
  ...
  // ── set_setting ──────────────────────────────────────────────────
  http.post('*/rest/v1/rpc/set_setting', () =>
  ...
  http.get('*/rest/v1/job_queue', () =>
    HttpResponse.json([
      { id: MOCK_JOB_ID, status: 'done', error_msg: JSON.stringify({ processed: 12, total: 12, failed_ids: [] }) },
    ]),
  ),
