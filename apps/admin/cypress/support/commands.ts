...
Cypress.Commands.add('loginAs', (role: AppRole) => {
  cy.session(
    ...
    () => {
      ...
      if (!email || !password) {
        throw new Error(`Missing Cypress env variables for role: ${role}. Ensure ${role}_email and ${role}_password are set in cypress.env.json or CI.`);
      }
      ...
      if (!supabaseUrl || !supabaseKey) {
         throw new Error('Missing Supabase config. Set supabase_url and supabase_anon_key in Cypress env.');
      }
