...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Playwright E2E tests pass (7 spec files / 31 tests: auth login+logout+token-version, user list & filters + lock/unlock + suspend duration validation + bulk-lock dialog/cancel + bulk-lock real submit + warn validation + real warn, courses create validation + create/delete with restore, courses enroll validation + enroll-then-delete-course restore, audit real chain verification, global app-lock settings, a11y audits, UX/RTL/keyboard regression) | QA    | ✅      | Run: `pnpm --filter @eduzone/admin exec playwright test`. **Confirmed green**: [E2E (Playwright)](https://github.com/mi9092921-alt/EduZone_dashboard/actions/runs/34320206324/job/102364929634) — 31 passed, zero retries. Gate live in `.github/workflows/e2e.yml` (`E2E_ENABLED=true`). |
...
>
> Cypress must **not** be removed (dependency, specs, or `cypress.env.json.example`) until all flows are ported and verified green in Playwright. Until then, both suites are required launch gates. **2 flows remain**, all still real Cypress-only files with no Playwright equivalent:
> - `notifications/notifications-flow.cy.ts`
> - `settings/maintenance-mode.cy.ts`
>
> `audit/verify-chain.cy.ts` was ported as new `audit.spec.ts` (real ChainVerifier button + live `activity_logs` + client-side `verifyHashChain`, accepting the genuine intact-or-tamper verdict; fully read-only). Honesty note: the port needed 3 CI runs -- two failed with an unmount-detach/timeout signature on this test (and the bulk test in the same runs) plus one page-state confusion, all unreproducible from static analysis (no rewrites, no error boundaries that unmount, no polling/realtime in either page). It then went 31/31 with zero retries. Both this test and the bulk test now carry embedded self-diagnostics (URL + page errors + failed requests printed on failure) so the next occurrence arrives with decisive data; the hash crypto itself is covered in unit tests regardless.
>
