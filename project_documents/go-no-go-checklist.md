...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Playwright E2E tests pass (5 spec files / 21 tests: auth login+logout+token-version, user list & filters + lock/unlock, global app-lock settings, a11y audits, UX/RTL/keyboard regression)                            | QA    | ✅      | Run: `pnpm --filter @eduzone/admin exec playwright test`. **Confirmed green**: [E2E (Playwright) #32](https://github.com/mi9092921-alt/EduZone_dashboard/actions/runs/34172510862/job/101895402046) — 21 passed (1.4m), commit `2d8ae17`. Gate live in `.github/workflows/e2e.yml` (`E2E_ENABLED=true`). |
...
>
> Cypress must **not** be removed (dependency, specs, or `cypress.env.json.example`) until all flows are ported and verified green in Playwright. Until then, both suites are required launch gates. **9 flows remain**, all still real Cypress-only files with no Playwright equivalent:
> - `users/suspend-user.cy.ts`
