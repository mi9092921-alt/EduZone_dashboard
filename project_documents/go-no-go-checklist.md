...
| 🔴 **P0: Lock/Unlock/Suspend/Ban and Terminate Sessions always fail** | Dev | ☐ | **Fix drafted (2026-09-05), pending audit sign-off + CI verification — not yet applied.** Root cause: `control_user_account` and `terminate_user_sessions` (`schema/07_functions.sql:2457,2394`) each gated on `public.user_has_permission(auth.uid(), ...)` / `p_user_id = auth.uid()`, but both are called only via the bare service_role client (`admin.rpc(...)`, `infrastructure/repos/user-admin.repository.ts`; `infrastructure/supabase/admin.ts` — "unauthenticated... bypasses RLS"), which carries no user JWT, so `auth.uid()` was always `NULL` there and both checks evaluated false unconditionally → `RAISE EXCEPTION 'PERMISSION_DENIED'` on every call. Caller-side authorization (`requirePermission('users.lock')`) was and is correct — purely the redundant internal check was broken. `issue_warning` was **not** affected (uses the request-scoped `createServerClient()`, `user-admin.repository.ts:164`). Reproduced live: [E2E (Playwright) #23](https://github.com/mi9092921-alt/EduZone_dashboard/actions/runs/33977117026) (`error-context.md` showed the dialog left open with "An unexpected error occurred" — that generic text is `InfrastructureError`'s deliberate, correctly-working masking design, not a bug, initially miscalled one here and corrected).<br><br>**First fix attempt (reverted):** adding an explicit `p_actor_id` param to `control_user_account`/`terminate_user_sessions` directly. Parsed clean under `pglast` but **broke the live DB migration** — run #25 (log pasted by the user; no run link given) failed with `function public.terminate_user_sessions(uuid, text) does not exist`, because `10_permissions.sql:498-500` has explicit `REVOKE/GRANT ... ON FUNCTION` statements pinned to the *old* 2-arg signature — a cross-file consistency `pglast` can't catch by construction (it parses one statement at a time). Fully reverted (`git checkout HEAD -- supabase/schema/07_functions.sql`) once found.<br><br>**Actual fix**: the codebase already has a correct, parallel `worker_*` function pair built for exactly this service-role calling pattern — `worker_control_user_account`/`worker_terminate_user_sessions` (`schema/07_functions.sql:4078,4210`) take the acting user explicitly as `p_initiator_id`, check permission/tenant against *that*, and are the same two RPCs `supabase/functions/bulk-worker/index.ts` already calls via its own service-role client (confirmed: `10_permissions.sql:515-523` grants service_role on the `worker_*` names only, not the plain ones). `user-admin.repository.ts`'s `controlAccount()`/`terminateSessions()` now call `worker_control_user_account`/`worker_terminate_user_sessions` instead — **zero schema changes**, no audit-flagged file touched at all. `IUserAdminRepository`/the two use-cases still thread `ctx.userId` through as `actorId` (now mapped to `p_initiator_id`), and both Server Actions still gained the `assertSameTenant` IDOR/BOLA guard `deleteUserAction` already had (a real, independent gap this surfaced — worker_terminate_user_sessions checks `users.write`, not `sessions.manage` like the unused direct RPC, and has no self-terminate exemption; doesn't affect this admin-only call site but flagged in `rpc-catalog.ts`). Catalog (`rpc-catalog.ts`) updated to stop pointing at the unused, broken direct RPCs and document the `worker_*` ones as the real owner. Still needs: human sign-off (touches privileged-RPC call sites even though not the schema itself) + a real CI run confirming `lock-user` (18/18) and the full migration both go green. |
