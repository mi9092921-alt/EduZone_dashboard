...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Playwright E2E tests pass (4 spec files / 18 tests: auth login+logout+token-version, user list & filters + lock/unlock, a11y audits, UX/RTL/keyboard regression)                                                       | QA    | ✅      | Run: `pnpm --filter @eduzone/admin exec playwright test`. **Confirmed green**: [E2E (Playwright) #31](https://github.com/mi9092921-alt/EduZone_dashboard/actions/runs/34170877175/job/101890788263) — 19 passed (18 chromium + 1 setup), no flaky, commit `ce9a71f` (verified via the run page's own commit tag — this is current HEAD, so it covers the trigger/self-logout SQL follow-up below too, not just the earlier worker_* switch). Gate live in `.github/workflows/e2e.yml` (`E2E_ENABLED=true`). |
...
>
> Cypress must **not** be removed (dependency, specs, or `cypress.env.json.example`) until all flows are ported and verified green in Playwright. Until then, both suites are required launch gates. Tracking: `lock-user` and the full P0 fix confirmed green (run #31, 19/19, commit `ce9a71f`) — port the remaining 10 flows, then re-run this checklist update to drop the Cypress row.
